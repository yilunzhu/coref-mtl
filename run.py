...
class Runner:
    def __init__(self, config_name, gpu_id=0, dataset='none', corpus=None, seed=None):
        ...
        # Set up data
        self.data = CorefDataProcessor(self.config, self.dataset, corpus=corpus)
            ...
            for doc_key, example in examples_train:
                ...
                if len(loss_during_accum) % grad_accum == 0:
                    ...
                    if len(loss_history) > 0 and len(loss_history) % conf['eval_frequency'] == 0:
                        f1, _, _ = self.evaluate(model, examples_dev, stored_info, len(loss_history), official=False, conll_path=self.config['conll_eval_path'], tb_writer=tb_writer)
                        if f1 > max_f1:
                            max_f1 = f1
    ...
    def evaluate(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None):
        ...
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            ...
            gold_sg_span_starts, gold_sg_span_ends = tensor_example[7].tolist(), tensor_example[8].tolist()
            gold_entities = tensor_example[12].tolist() if tensor_example[12] is not None else None
            gold_infstats = tensor_example[13].tolist() if tensor_example[13] is not None else None
            tensor_example = list(tensor_example[:7])  # Strip out gold
            example_gpu = [d.to(self.device) for d in tensor_example]
            with torch.no_grad():
                _, _, _, span_starts, span_ends, antecedent_idx, antecedent_scores, entity_preds, infstat_preds = model(*example_gpu)

            # get gold and pred (start, end, label) triplets
            # entity_preds = entity_preds.tolist() if entity_preds is not None else None
            # infstat_preds = infstat_preds.tolist() if infstat_preds is not None else None
            if entity_preds is not None:
                entity_preds_tuple = [(span_starts[idx].tolist(), span_ends[idx].tolist(), entity_preds[idx].tolist()) for idx in range(len(span_starts))]
                entity_gold_tuple = [(gold_sg_span_starts[idx], gold_sg_span_ends[idx], gold_entities[idx]) for idx in range(len(gold_entities))]
            else:
                entity_gold_tuple, entity_preds_tuple = None, None
            if infstat_preds is not None:
                infstat_preds_tuple = [(span_starts[idx].tolist(), span_ends[idx].tolist(), infstat_preds[idx].tolist()) for idx in range(len(span_starts))]
                infstat_gold_tuple = [(gold_sg_span_starts[idx].tolist(), gold_sg_span_ends[idx].tolist(), gold_infstats[idx]) for idx in range(len(gold_infstats))]
            else:
                infstat_gold_tuple, infstat_preds_tuple = None, None

            span_starts, span_ends = span_starts.tolist(), span_ends.tolist()
            antecedent_idx, antecedent_scores = antecedent_idx.tolist(), antecedent_scores.tolist()
            predicted_clusters = model.update_evaluator(span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, entity_preds_tuple, entity_gold_tuple, coref_evaluator, mention_evaluator)
            doc_to_prediction[doc_key] = predicted_clusters

        mp, mr, mf, mr_bdry = mention_evaluator.get_prf()
        mention_metrics = {'Entity_Eval_Avg_Precision': mp * 100, 'Entity_Eval_Avg_Recall': mr * 100, 'Entity_Eval_Avg_F1': mf * 100,
                           'Entity_Eval_Avg_Recall_Gold_Boundary': mr_bdry * 100
                           }
        for name, score in mention_metrics.items():
        ...
        return f * 100, metrics, mention_metrics
