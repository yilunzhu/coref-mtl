...
class CorefModel(nn.Module):
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        if conf['model_type'] == 'fast':
            ...
            candidate_idx_sorted_by_score = torch.argsort(candidate_mention_scores+candidate_sg_scores, descending=True).tolist()
            candidate_starts_cpu, candidate_ends_cpu = candidate_starts.tolist(), candidate_ends.tolist()
        ...
        if not do_loss:
            ...
            entity_preds = torch.argmax(top_span_entity_logits, dim=1) if conf['mtl_entity'] else None
            infstat_preds = torch.argmax(top_span_infstat_logits, dim=1) if conf['mtl_infstat'] else None
            return candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts, top_span_ends, top_antecedent_idx, top_antecedent_scores, entity_preds, infstat_preds
        ...
        if conf['model_type'] != 'fast' and conf['mtl_entity']:
            ...
            entity_preds = torch.argmax(top_span_entity_logits, dim=1)
        ...
        if conf['model_type'] != 'fast' and conf['mtl_infstat']:
            ...
            infstat_preds = torch.argmax(top_span_infstat_logits, dim=1)
        ...
        return [candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts, top_span_ends,
                top_antecedent_idx, top_antecedent_scores, entity_preds, infstat_preds], loss
    ...
    def update_evaluator(self, span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, pred_entity, gold_entity, coref_evaluator, mention_evaluator):
        ...
        mention_evaluator.update(pred_entity, gold_entity)
